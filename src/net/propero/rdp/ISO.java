...
public abstract class ISO {
	static Logger logger = Logger.getLogger(ISO.class.getName());
	...
	private RdpPacket_Localised receiveMessage(int[] type) throws IOException,
		...
		next_packet: while (true) {
			...
			if ((version & 3) == 0) {
				...
				SessionManager.getInstance().get(sessionId).rdp.rdp5_process(s, (version & 0x80) != 0);
	...
	void send_connection_request() throws IOException {
		...
		String uname_org = SessionManager.getInstance().getOption(sessionId).username;
		String uname = uname_org;
		
		if (uname.length() > 9)
			uname = uname.substring(0, 9);
		int length = 11 + (uname_org.length() > 0 ? ("Cookie: mstshash="
				.length()
				+ uname.length() + 2) : 0) + 8;
