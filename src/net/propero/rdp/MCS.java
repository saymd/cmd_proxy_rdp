...
public class MCS {
	static Logger logger = Logger.getLogger(MCS.class.getName());
	...
	public MCS(VChannels channels) {
		...
		IsoLayer = new ISO_Localised(channels.getSessionId());
	}
	...
	public void receiveConnectResponse(RdpPacket_Localised data)
		...
		SessionManager.getInstance().get(channels.getSessionId()).secure.processMcsData(buffer);
