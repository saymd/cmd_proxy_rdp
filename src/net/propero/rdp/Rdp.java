/* Rdp.java ( removed gui, screen related logics )
...
public class Rdp {
	...
	protected static Logger logger = Logger.getLogger(Rdp.class.getName());
...
//	private RdesktopFrame frame = new RdesktopFrame();
...
//	private RdesktopCanvas surface = null;
	...
	private RdesktopSession session;
...
//		}
	...
	public Rdp(VChannels channels) {
		session = SessionManager.getInstance().get(channels.getSessionId()); 
		this.SecureLayer = new Secure(channels);
		session.secure = SecureLayer;
		this.orders = new Orders();
		this.cache = new Cache();
		orders.registerCache(cache);
		options = session.getOption();
	}
	...
	private void sendConfirmActive() throws RdesktopException, IOException,
		...
		data.setLittleEndian16(session.mcs.getUserID() /* McsUserID() */+ 1001);
		...
		session.secure.send(data, sec_flags);
	}
	...
	public void sendInput(int time, int message_type, int device_flags,
		...
		try {
			...
			if (session.rdp.isConnected())
				session.error(r, session.rdp, true);
		} catch (CryptoException c) {
			if (session.rdp.isConnected())
				session.error(c, session.rdp, true);
		} catch (IOException i) {
			if (session.rdp.isConnected())
				session.error(i, session.rdp, true);
		}
	}
